...
The original scope and architecture are recorded in [SCOPE.md](SCOPE.md) and [HLD.md](HLD.md). The current workflow generates deployment files once and leaves them for the project to maintain. Cloud resource setup and pipeline execution are later workflows.
...
| Configuration         | Answers are validated in memory and discarded after setup. No answers file is read or generated.                                                                                                                 |
| File ownership        | The project owns its deployment files. No persistent manifest or stale-file deletion. Different existing files require review.                                                                                   |
...
The pinned Node defaults come from the [Node.js release index](https://nodejs.org/dist/index.json). The selected versions are written directly into the Dockerfile. Updating them is a deliberate code change and requires fixture verification.
...
The test suite uses temporary Git repositories to check dirty worktrees, submodules, unresolved conflicts, cancellation, existing-file review, absence of saved generator state, preservation of old metadata and unrelated files, concurrent changes, failed hooks, exact commit contents and identical output. Configuration and renderer tests cover branch mapping, static/SSR detection, engine constraints, environment handling and generated document validation.
